function Controller() {
    ...
    var limit = 20;
    var ready = true;
    var baseUrl = "http://miuniversidad-site.dev01.icti.es/api/otros/lista/";
    var xhr = Ti.Network.createHTTPClient({
        onload: function() {
            json = JSON.parse(this.responseText);
            for (i = 0; json.length > i; i++) otroChild.push(Alloy.createModel("otro", {
                id: json[i].id,
                name: json[i].name,
                icon: json[i].image,
                url: json[i].url
            }));
            for (var i = 0; otroChild.length > i; i++) {
